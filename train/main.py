...
def main():
    ...
    with tf.Session() as sess:
        ...
        for episode in range(FLAGS.max_episode_count):
            ...
            while not done:
                ...
                if len(replay_buffer) > FLAGS.batch_size:
                    ...
                    if FLAGS.step_verbose and step_count % FLAGS.step_verbose_count == 0:
                        logger.info(" - step_count : "+str(step_count)+", reward: "+str(e_reward)+" loss: "+str(loss))
            ...
            logger.info("Episode: "+str(episode)+" reward: "+str(e_reward)+" loss: "+str(model_loss)+"  consecutive_"+str(consecutive_len)+"_avg_reward: "+str(avg_reward)+"")
            ...
            if len(last_n_game_reward) == last_n_game_reward.maxlen:
                ...
                if config.solving_criteria and  avg_reward > (config.solving_criteria[1]):
                    logger.info("Game Cleared in"+str(episode)+" episodes with avg reward"+str(avg_reward))
                    break

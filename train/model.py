...
class Custom_CNNv0:
    ...
    def build_network(self) -> None:
        conv1 = tf.compat.v1.layers.conv1d(
            self.X, 16, kernel_size=3, padding="same", activation=tf.nn.relu)
        pool1 = tf.compat.v1.layers.max_pooling1d(inputs=conv1, pool_size=2, strides=2)
        conv2 = tf.compat.v1.layers.conv1d(
            pool1, 32, kernel_size=3, padding="same", activation=tf.nn.relu)
        pool2 = tf.compat.v1.layers.max_pooling1d(inputs=conv2, pool_size=2, strides=2)
        conv3 = tf.compat.v1.layers.conv1d(
            pool2, 64, kernel_size=3, padding="same", activation=tf.nn.relu)
        pool3 = tf.compat.v1.layers.max_pooling1d(inputs=conv3, pool_size=2, strides=2)
        pool3_flat = tf.reshape(pool3, [-1, 4 * 64])

        net = tf.compat.v1.layers.dense(pool3_flat, 256)
        net = tf.compat.v1.layers.dense(net, 32)
        net = tf.compat.v1.layers.dense(net, self.num_classes)
...
class Custom_CNNv2:
    ...
    def build_network(self) -> None:
        ...
        net = tf.compat.v1.layers.dense(pool2_flat, 1024)
        net = tf.compat.v1.layers.dense(pool2_flat, 512)
        net = tf.compat.v1.layers.dense(net, self.num_classes)

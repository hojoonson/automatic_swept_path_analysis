...
from dqn import DeepQNetwork, update_state_and_get_reward
...
class Simulation:
    ...
    def run(self):
        ...
        result_path = os.path.join(result_path, 'reward_and_loss')
        os.makedirs(result_path, exist_ok=True)
        ...
        with tf.compat.v1.Session() as sess:
            ...
            for episode in range(FLAGS.max_episode_count):
                #set road image!
                random_index = int(episode/self.map_updatecount)%len(self.roadimage_path)
                self.util.imagefile=self.roadimage_path[random_index]['image_path']
                self.startx=self.roadimage_path[random_index]['startx']
                self.starty=self.roadimage_path[random_index]['starty']
                ...
                while not self.done:
                    ...
                    reward = update_state_and_get_reward(action, vehicle)

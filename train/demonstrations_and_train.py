...
class Game:
    ...
    def run(self):
        ...
        lossresult_path = "demon_and_train_output:"+str(self.outputsize) + "_f" + str(FLAGS.frame_size) + "_transport_"+str(
            self.vehicle)+"_model_" + FLAGS.model_name+"_"+FLAGS.checkpoint_path + "global_step"
        ...
        with tf.Session() as sess:
            ...
            for episode in range(FLAGS.max_episode_count):
                ...
                if len(stack_list) >= self.random_candidate and episode % self.map_updatecount != 0:
                    randomstate = random.sample(
                        stack_list, self.random_candidate)
                ...
                while not self.done:
                    ...
                    if len(replay_buffer) > FLAGS.batch_size:
                        ...
                        if FLAGS.step_verbose and step_count % FLAGS.step_verbose_count == 0:
                            print(" - step_count : "+str(step_count)+", reward: "+str(e_reward) +
                                  " ,loss: "+str(loss)+", global_step: "+str(global_step))
                    ...
                    if global_step % FLAGS.save_step_count == 0:
                        checkpoint_path = "Pantechnicon_Removals_Van_demon_and_train_output:"+str(self.outputsize) + "_f" + str(
